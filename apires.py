...
def handle_compressed_data(response: requests.Response) -> str:
    ...
    if 'application/zip' in response.headers.get('Content-Type', ''):
        ...
        # Read the content of the CSV file inside the zip using different encodings
        with zip_file.open(zip_file_name) as csvfile:
            ...
            try:
                ...
                csv_content = csvfile.read().decode('ISO-8859-1')
            except UnicodeDecodeError:
                print("  ⚠  'ISO-8859-1' failed, trying 'Windows-1252' encoding …")
                csv_content = csvfile.read().decode('Windows-1252')
...
def download_indicator_reporter(indicator_code: str, reporter_code: str) -> None:
    ...
    while True:
        ...
        try:
            csv_content = handle_compressed_data(response)
    ...
    # Combine all data from different pages into a single CSV file
    csv_file_name = f"{indicator_code}_{reporter_code}_data_{RUN_DATE}.csv"
